...
class QuizSystem {
    ...
    renderOptions(question) {
        ...
        // Shuffle options with correct index tracking
        ...
        // Animation directions (top, right, bottom, left)
        const directions = [
            { x: 0, y: -50 },    // top
            { x: 50, y: 0 },     // right
            { x: 0, y: 50 },     // bottom
        ...
        shuffledOptions.forEach((option, index) => {
            ...
            btn.addEventListener('click', () => this.selectOption(option.originalIndex, question));

            this.optionsContainer.appendChild(btn);

            // Animated entrance from different directions
    ...
    handleCorrectAnswer(selectedIndex) {
        const selectedBtn = document.querySelector(`.option-btn[data-original-index="${selectedIndex}"]`);
    ...
    handleWrongAnswer(selectedIndex, correctIndex) {
        const selectedBtn = document.querySelector(`.option-btn[data-original-index="${selectedIndex}"]`);
        const correctBtn = document.querySelector(`.option-btn[data-original-index="${correctIndex}"]`);
    ...
    triggerHaptic(type) {
        ...
        try {
            if (type === 'success') {
                ...
                // Single strong pulse for wrong answer
